...
I never stopped caring about weather. Now I'm spending my time building, learning, and thinking about what comes next.

---

## 🌦️ What I'm Building

A 0-to-1 AI-native consumer product. I'm not sharing the specifics publicly yet, but the work covers data ingestion and enrichment, LLM orchestration, agentic workflows, voice synthesis, prompt engineering, and evaluation frameworks designed to make AI behave reliably when real users depend on it.

The build is grounded in Spec-Driven Development and a disciplined approach to iteration, quality, and creative correctness. More to come when the product is ready.

### The Technical Foundation

The platform is built on a Node.js/TypeScript backend with a Next.js 15 frontend. The data pipeline ingests from public data sources, enriches the results through a tiered enrichment layer, and serves them through a Redis cache with type-specific TTLs (real-time data refreshes faster than forecast data). When upstream APIs fail, the system serves stale data with transparency flags rather than returning errors, because people rely on the product for real decisions and a slightly stale result beats no result.

Key architecture decisions:

| Decision | What we chose | Why |
|----------|--------------|-----|
| Data sources | Public APIs over commercial | Free, high-quality, removes per-request cost as a scaling constraint |
| Failure mode | Stale cache with `isStale` flag | Users need the product working even when upstream is down |
| Upstream protection | Circuit breaker pattern | Prevents request floods during outages |
| Data enrichment | Additive, not silent correction | Transparency over magic, users see the context |
...
- The gap between data available and data useful
...
| | |
|---|---|
| **Epic Forecast** | Founder and Principal Consultant. Product management, CX strategy, and enterprise application consulting while building a 0-to-1 AI-native consumer product |
